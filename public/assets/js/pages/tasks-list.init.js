...
var table = document.getElementById("tasksTable"); // save all tr
...
function SearchData() {
  ...
  tasksList.filter(function (data) {
    ...
    if (new Date(data.values().due_date.slice(0, 12)) >= new Date(date1) && new Date(data.values().due_date.slice(0, 12)) <= new Date(date2)) {
      dateFilter = true;
    } else {
      dateFilter = false;
    }
