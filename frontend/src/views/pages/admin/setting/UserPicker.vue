    ...
    <ModuleHeader>
      ...
      <template v-slot:desc>
        <v-alert color="cyan" border="left" colored-border type="info">
          User dengan role PICKER bertanggungjawab terhadap proses
          ...
          >
            <template v-slot:top>
              <v-toolbar flat color="white">
                <v-toolbar-title>DAFTAR USERS PICKER</v-toolbar-title>
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      ...
                      class="ma-2"
                      :disabled="btnLoading"
                      @click.stop="showDialogTambahUserPicker"
                    >
                      <v-icon>mdi-plus</v-icon>
                    </v-btn>
                  </template>
                  <span>Tambah User Picker</span>
                </v-tooltip>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      syncPermission() {
        this.$root.$confirm
          .open(
            ...
            "Sinkronisasi hanya untuk user dalam role picker, bila user memiliki role lain akan terhapus permission-nya ?",
      ...
      showDialogTambahUserPicker: async function() {
        this.dialog = true;				
      },
      ...
      save() {
        if (this.$refs.frmdata.validate()) {
          ...
          if (this.editedIndex > -1) {
            this.$ajax
              .post(
                "/setting/pengguna/picker/" + this.editedItem.id,
                {
                  _method: "PUT",
        ...
        this.$root.$confirm
          ...
          .then(confirm => {
            if (confirm) {
              ...
              this.$ajax
                .post(
                  "/setting/pengguna/picker/" + item.id,
                  {
                    _method: "DELETE",
                  },
                  {
