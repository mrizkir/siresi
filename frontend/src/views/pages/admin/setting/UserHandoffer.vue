    ...
    <ModuleHeader>
      ...
      <template v-slot:desc>
        <v-alert color="cyan" border="left" colored-border type="info">
          User dengan role HAND OFFER bertanggungjawab terhadap proses
          ...
          >
            <template v-slot:top>
              <v-toolbar flat color="white">
                <v-toolbar-title>DAFTAR USERS HAND OFFER</v-toolbar-title>
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      ...
                      class="ma-2"
                      :disabled="btnLoading"
                      @click.stop="showDialogTambahUserHandOffer"
                    >
                      <v-icon>mdi-plus</v-icon>
                    </v-btn>
                  </template>
                  <span>Tambah User Hand Offer</span>
                </v-tooltip>
      ...
      syncPermission() {
        this.$root.$confirm
          .open(
            ...
            "Sinkronisasi hanya untuk user dalam role handoffer, bila user memiliki role lain akan terhapus permission-nya ?",
      ...
      save() {
        if (this.$refs.frmdata.validate()) {
          ...
          if (this.editedIndex > -1) {
            this.$ajax
              .post(
                "/setting/pengguna/handoffer/" + this.editedItem.id,
                {
                  _method: "PUT",
            ...
            this.$ajax
              .post(
                "/setting/pengguna/handoffer/store",
                {
                  name: this.editedItem.name,
                  email: this.editedItem.email,
                  nomor_hp: this.editedItem.nomor_hp,
        ...
        this.$root.$confirm
          ...
          .then(confirm => {
            if (confirm) {
              ...
              this.$ajax
                .post(
                  "/setting/pengguna/handoffer/" + item.id,
                  {
                    _method: "DELETE",
                  },
                  {
    ...
    computed: {
      formTitle() {
        return this.editedIndex === -1
          ? "TAMBAH USER HAND OFFER"
          : "EDIT USER HAND OFFER";
      },
      ...mapGetters("auth", {
