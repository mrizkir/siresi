		...
		<ModuleHeader>
			...
			<template v-slot:desc>
				<v-alert color="cyan" border="left" colored-border type="info">
					Daftar aksi-aksi terhadap sebuah modul. Format penulisan permission,
					NAMAMODULE atau NAMA MODULE. Nama Permission tighly coupling dengan
					kode sumber.
				</v-alert>
			</template>
		</ModuleHeader>
								...
								<v-dialog v-model="dialog" max-width="500px" persistent>
									<template v-slot:activator="{ on }">
										<v-btn
											...
											class="ma-2"
											v-on="on"
											:disabled="!CAN_ACCESS('PERMISSIONS_STORE')"
										>
		...
		data: () => ({
			...
			rule_permission_name: [
				...
				value =>
					...
					"Nama Permission hanya boleh angka,huruf,dan tanda -",
			],
		}),
		methods: {
			initialize() {
			...
			deleteItem(item) {
				this.$root.$confirm
					.open(
						"Delete",
						"Apakah Anda ingin menghapus permission " + item.name + " ?",
						{ color: "red" }
					)
					.then(confirm => {
						if (confirm) {
