...
<script>
	export default {
		...
		methods: {
			initialize() {
				this.$ajax
					.get("/setting/users/" + this.user.id + "/roles", {
						headers: {
			...
			save() {
				...
				this.$ajax
					.post(
						"/setting/users/storeuserpermissions",
						{
			...
			revoke(item) {
				...
				this.$ajax
					.post(
						"/setting/users/revokeuserpermissions",
						{
							user_id: this.user.id,
							name: item.name,
						},
		...
		watch: {
			async role_name(val) {
				if (val) {
					...
					await this.$ajax
						.get(
							"/setting/pengguna/rolesbyname/" + this.role_name + "/permission",
							{
								headers: {
									Authorization: this.$store.getters["auth/Token"],
								},
